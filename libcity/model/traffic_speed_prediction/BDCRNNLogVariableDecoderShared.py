...
from libcity.model.traffic_speed_prediction.BDCRNNBase import BDCRNNBase, EncoderSigmaModel, DecoderSigmaModel
from libcity.model.traffic_speed_prediction.layers.functions import count_parameters


class BDCRNNLogVariableDecoderShared(BDCRNNBase):
    def __init__(self, config, data_feature):
        super(BDCRNNLogVariableDecoderShared, self).__init__(config, data_feature)
    ...
    def forward(self, batch, batches_seen=None):
        ...
        log_sigma_0 = self.decoder_sigma(encoder_hidden_state, labels, batches_seen=batches_seen)
        # (self.output_window, batch_size, self.num_nodes * self.output_dim)
        ...
        log_sigma_0 = log_sigma_0.view(self.output_window, batch_size, self.num_nodes, self.output_dim).permute(1, 0, 2, 3)
        return outputs, log_sigma_0

    def calculate_loss(self, batch, batches_seen=None, num_batches=1):
        y_true = batch['y']
        y_predicted, log_sigma_0 = self.forward(batch, batches_seen)
