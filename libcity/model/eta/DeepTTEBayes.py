...
from libcity.model.eta.layers.rand_conv1d import RandConv1d
from libcity.model.eta.layers.rand_linear import RandLinear
...
class RandGeoConv(nn.Module):
    def __init__(self, kernel_size, num_filter, data_feature={}, device=torch.device('cpu')):
        ...
        self.process_coords = RandLinear(4, 16)
        self.conv = RandConv1d(16, self.num_filter, self.kernel_size)
...
class SpatioTemporal(nn.Module):
    ...
    def __init__(self, attr_size, kernel_size=3, num_filter=32, pooling_method='attention',
        ...
        if pooling_method == 'attention':
            self.attr2atten = RandLinear(attr_size, hidden_size)
...
class EntireEstimator(nn.Module):
    def __init__(self, input_size, num_final_fcs, hidden_size=128):
        ...
        self.input2hid = RandLinear(input_size, hidden_size)

        self.residuals = nn.ModuleList()
        for i in range(num_final_fcs):
            self.residuals.append(RandLinear(hidden_size, hidden_size))

        self.hid2out = RandLinear(hidden_size, 1)
...
class LocalEstimator(nn.Module):
    def __init__(self, input_size, eps=10):
        ...
        self.input2hid = RandLinear(input_size, 64)
        self.hid2hid = RandLinear(64, 32)
        self.hid2out = RandLinear(32, 1)
...
class DeepTTEBayes(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        super(DeepTTEBayes, self).__init__(config, data_feature)
        self.config = config
        self.data_feature = data_feature
